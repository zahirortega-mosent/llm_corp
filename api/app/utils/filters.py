...
def period_label(period: str | None) -> str | None:
    ...
    if not period:
        return None
    month = SPANISH_MONTH_BY_NUMBER.get(int(period[5:7]), period[5:7])
    return f"{month} {period[:4]}"
...
def _periods_for_month(month: int, periods: list[str]) -> list[str]:
    return [period for period in periods if period[5:7] == f"{month:02d}"]
...
def _parse_explicit_numeric_period(question_norm: str) -> str | None:
    ...
    numeric_alt = re.search(r"\b(0?[1-9]|1[0-2])[-/](20\d{2})\b", question_norm)
    if numeric_alt:
        return f"{int(numeric_alt.group(2)):04d}-{int(numeric_alt.group(1)):02d}-01"
    return None


def _parse_month_year_period(question_norm: str) -> tuple[str | None, dict[str, Any]]:
    for month_name, month_num in MONTHS.items():
        # enero 2026, enero de 2026, enero del 2026
        pattern = "\\b" + re.escape(month_name) + "\\b(?:\\s+(?:de|del))?\\s+(20\\d{2})\\b"
        match = re.search(pattern, question_norm)
        if match:
            year = int(match.group(1))
            period = f"{year:04d}-{month_num:02d}-01"
            return period, {
                ...
                "year": year,
                "ambiguous": False,
                "resolved_period_label": period_label(period),
            }
    return None, {}
        ...
        if re.search("\\b" + re.escape(month_name) + "\\b", question_norm):
            ...
            if len(candidates) == 1:
                return candidates[0], {
                    ...
                    "resolved_period_label": period_label(candidates[0]),
                }
            if len(candidates) > 1:
                return None, {
                    ...
                    "candidate_period_labels": [period[:7] for period in candidates],
...
def _parse_periods_list(question_norm: str, periods: list[str]) -> list[str]:
    # Caso acotado: "enero y febrero 2026" / "enero, febrero y marzo de 2026".
    ...
    if len(month_hits) < 2:
        return []
    return [f"{year:04d}-{month_num:02d}-01" for month_num in sorted(set(month_hits))]
...
def _parse_bank(question_norm: str) -> str | None:
...
def parse_question_filters(question: str, metadata: Dict[str, Any]) -> Dict[str, Any]:
    ...
    period_list: list[str] = _parse_quarter_periods(question_norm) or _parse_periods_list(question_norm, periods)
    ...
    if period_list:
        resolution.update({
            ...
            "period_labels": [period[:7] for period in period_list],
        })
    else:
        ...
        if numeric_period:
            ...
            resolution.update({
                ...
                "period_confidence": 1.0,
                "ambiguous": False,
                "resolved_period_label": period_label(detected_period),
            })
        else:
    ...
    if not period_list and "este mes" in question_norm and periods:
    ...
    detected_bank = _parse_bank(question_norm)
