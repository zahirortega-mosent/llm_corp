...
movimientos_parseados AS (
    SELECT
        mb.*,
        UPPER(CONCAT(
            COALESCE(mb.nom_cta_mayor, ''), ' ',
            COALESCE(mb.nombre_cuenta_fin, ''), ' ',
            COALESCE(mb.reference, ''), ' ',
            COALESCE(mb.concept, ''), ' ',
            COALESCE(mb.description, ''), ' ',
            COALESCE(mb.cliente, ''), ' ',
            COALESCE(mb.diario_fin, ''), ' ',
            COALESCE(mb.move_name, '')
        )) AS bank_text,
        NULLIF(REPLACE(REPLACE(REPLACE(REPLACE(LTRIM(RTRIM(COALESCE(mb.codigo_cuenta_fin, ''))), ' ', ''), '-', ''), '.', ''), '/', ''), '') AS codigo_cuenta_digits,
        NULLIF(REPLACE(REPLACE(REPLACE(REPLACE(LTRIM(RTRIM(COALESCE(mb.nombre_cuenta_fin, ''))), ' ', ''), '-', ''), '.', ''), '/', ''), '') AS nombre_cuenta_digits,
        NULLIF(REPLACE(REPLACE(REPLACE(REPLACE(LTRIM(RTRIM(COALESCE(mb.reference, ''))), ' ', ''), '-', ''), '.', ''), '/', ''), '') AS reference_digits,
        REVERSE(LEFT(REVERSE(COALESCE(mb.nom_cta_mayor, '')), PATINDEX('%[^0-9]%', REVERSE(COALESCE(mb.nom_cta_mayor, '')) + 'X') - 1)) AS trailing_digits_nom,
        REVERSE(LEFT(REVERSE(COALESCE(mb.nombre_cuenta_fin, '')), PATINDEX('%[^0-9]%', REVERSE(COALESCE(mb.nombre_cuenta_fin, '')) + 'X') - 1)) AS trailing_digits_name,
        REVERSE(LEFT(REVERSE(COALESCE(mb.reference, '')), PATINDEX('%[^0-9]%', REVERSE(COALESCE(mb.reference, '')) + 'X') - 1)) AS trailing_digits_ref
...
movimientos_con_cuenta AS (
    SELECT
        mp.*,
        CASE
            WHEN mp.bank_text LIKE '%SANTANDER%' THEN 'SANTANDER'
            WHEN mp.bank_text LIKE '%SCOTIA INVERT%' OR mp.bank_text LIKE '%SCOTIABANK INVERT%' THEN 'SCOTIABANK INVERT'
            WHEN mp.bank_text LIKE '%SCOTIA%' OR mp.bank_text LIKE '%SCOTIABANK%' THEN 'SCOTIABANK'
            WHEN mp.bank_text LIKE '%BANBAJIO%' OR mp.bank_text LIKE '%BAJIO%' THEN 'BANBAJIO'
            WHEN mp.bank_text LIKE '%BANAMEX%' OR mp.bank_text LIKE '%CITIBANAMEX%' OR mp.bank_text LIKE '%CITI BANAMEX%' THEN 'BANAMEX'
            WHEN mp.bank_text LIKE '%BANORTE%' THEN 'BANORTE'
            WHEN mp.bank_text LIKE '%BANREGIO%' THEN 'BANREGIO'
            WHEN mp.bank_text LIKE '%BBVA%' OR mp.bank_text LIKE '%BANCOMER%' THEN 'BBVA'
            ELSE NULL
        END AS bank_inferred,
        CASE
            WHEN mp.codigo_cuenta_digits NOT LIKE '%[^0-9]%' AND LEN(mp.codigo_cuenta_digits) BETWEEN 6 AND 18 THEN mp.codigo_cuenta_digits
            WHEN mp.nombre_cuenta_digits NOT LIKE '%[^0-9]%' AND LEN(mp.nombre_cuenta_digits) BETWEEN 6 AND 18 THEN mp.nombre_cuenta_digits
            WHEN mp.reference_digits NOT LIKE '%[^0-9]%' AND LEN(mp.reference_digits) BETWEEN 6 AND 18 THEN mp.reference_digits
            WHEN LEN(NULLIF(mp.trailing_digits_name, '')) BETWEEN 6 AND 18 THEN mp.trailing_digits_name
            WHEN LEN(NULLIF(mp.trailing_digits_nom, '')) BETWEEN 6 AND 18 THEN mp.trailing_digits_nom
            WHEN LEN(NULLIF(mp.trailing_digits_ref, '')) BETWEEN 6 AND 18 THEN mp.trailing_digits_ref
            ...
            WHEN LEN(NULLIF(mp.trailing_digits_ref, '')) >= 4 THEN mp.trailing_digits_ref
            ELSE NULL
...
matches AS (
        ...
        ROW_NUMBER() OVER (
            ...
            ORDER BY
                CASE
                    ...
                    WHEN LEN(ISNULL(mc.account_number_extracted, '')) >= 6
                         AND RIGHT(REPLACE(ISNULL(c.clabe, ''), ' ', ''), LEN(mc.account_number_extracted)) = mc.account_number_extracted THEN 2
                    WHEN LEN(ISNULL(mc.account_number_extracted, '')) >= 4
                         AND RIGHT(c.no_cuenta, 4) = RIGHT(mc.account_number_extracted, 4) THEN 3
                    WHEN LEN(ISNULL(mc.account_number_extracted, '')) >= 4
                         AND RIGHT(REPLACE(ISNULL(c.clabe, ''), ' ', ''), 4) = RIGHT(mc.account_number_extracted, 4) THEN 4
                    ELSE 99
                END,
                c.fecha_final DESC,
                c.id DESC
    ...
    INNER JOIN movimientos_con_cuenta mc
       ...
       AND (
            ...
            OR (
                LEN(ISNULL(mc.account_number_extracted, '')) >= 6
                AND RIGHT(REPLACE(ISNULL(c.clabe, ''), ' ', ''), LEN(mc.account_number_extracted)) = mc.account_number_extracted
            )
            OR (
            ...
            OR (
                LEN(ISNULL(mc.account_number_extracted, '')) >= 4
                AND RIGHT(REPLACE(ISNULL(c.clabe, ''), ' ', ''), 4) = RIGHT(mc.account_number_extracted, 4)
            )
       )
       AND (
